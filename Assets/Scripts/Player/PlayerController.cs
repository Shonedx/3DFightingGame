...
public class PlayerController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        movement = new Movement(transform, rb, transform.forward);
        movement.Init();
        if(movement==null)
    ...
    private void Jump()
    {
        bool state = gameInput.GetToJump();
        if (state)
        {
            movement.Jump(state);
            StartCoroutine(movement.AfterJump());
        }
        
    }
    private void StartCrouch()=>movement.StartCrouch(gameInput.GetCrouchPerformed());
    private void StopCrouch()=>movement.StopCrouch(gameInput.GetCrouchCanceled());
    private void Sprint()=>movement.Sprint(gameInput.GetToSprint());
    private void Slide()
    {
        bool state= gameInput.GetToSlide();
        if (state)
        {
            movement.Slide(state);
            StartCoroutine(movement.SlideDone());
        }
    }
    private void Move()=>movement.Move();
    public bool GetIfMoving()
    {
        return movement.GetIfMoving(); 
    } 
    private void SetMoveDir()
    {
        Vector2 inputVector2 = gameInput.GetMovementNormalized();
        Vector3 inputMoveDir = transform.forward * inputVector2.y + transform.right * inputVector2.x;
        movement.TargetDir= Vector3.ProjectOnPlane(inputMoveDir.normalized, movement.Hit.normal);
    }
    private void Update()
    {
        SetMoveDir();
        ...
        {
            gun.Reload();
            CommandLineManager.ShowStatusUpdate("Reload!");
        }
        Move();
    ...
    //    {
    //        Jump();
    //        CommandLineManager.ShowStatusUpdate("Jump!");
    //    }
    //    if(gameInput.GetCrouchPerformed()) //检测下蹲
    //    {
    //        Crouch();
    //        CommandLineManager.ShowStatusUpdate("Crouch!");
    //    }
    ...
    //    CommandLineManager.ShowStatusUpdate("Sprint!");
    //}
    //#endregion
    //#region Crouch
    //private void Crouch()
